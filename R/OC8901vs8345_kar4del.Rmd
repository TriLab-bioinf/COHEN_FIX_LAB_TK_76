---
title: "Comparing OC8901 and OC8345 - kar4 only (Hernan Revision)"
...
```{r}
# # Import read counts table into read_counts variable
# read_counts <- read.table(file = "read_counts_table_comp8901vs8345_kar4delonly.txt",  
#                           header = TRUE, # First line contains column headers 
#                           row.names = 1, # Column 1 contains row names
#                           sep = "\t")   # Column delimiter is a tab (\t)
# 
...
# 
# # Read metadata table
# metadata <- read.table(file = "metadata_OC8901vsOC8345_kar4delonly.txt", 
#                        header = TRUE, 
#                        row.names = 1,
#                        sep = "\t")
# 
...
# metadata$sample_id <- c("AY31", "AY32" , "AY33" , "AY34" , "AY1", "AY2", "AY3", "AY4")
# 
...
dir.create(path = "./Plots_OC8901vsOC8345_kar4del", showWarnings = FALSE)
...
ggsave2("Plots_OC8901vsOC8345_kar4del/barplot_read_counts_per_sample_AMANDA.pdf", plot = readcounts.p)
...
ggsave2(filename = "./Plots_OC8901vsOC8345_kar4del/heat_map_AMANDA.pdf", plot = hm.p)

print(hm.p)

```

**Figure 2:** kar4 samples cluster by treatment.
...
ggsave(paste0("Plots_OC8901vsOC8345_kar4del/pca_by_group_PC",principal_components[1],principal_components[2],"_AMANDA.pdf"), plot = pca.p)
print(pca.p)
```

**Figure 3:** Samples group mainly by alpha factor treatment (PC1), but there is a batch effect based on experiment number.
...
### Differential expression analysis -kar4∆:

**1- The effect of treatment across all kar4∆ samples (the main effect) **
...
dir.create("./Results_kar4del_OC8901vsOC8345", showWarnings = FALSE)
...
for (example_name in names(DE_results_list)){
  ...
  write.table(x = as.data.frame(DE_results_list[[example_name]]), 
            file = paste0("./Results_kar4del_OC8901vsOC8345/DE_",example_name,".txt"),
            sep = "\t", 
            col.names = NA)
}
```