...
get_gene_names <- function(A){
  if(length(A) > 0){
    ...
    get_gene_names_from_gene_ids(ensemble_ids = my_ensembl_ids, 
                                 ...
                                 look_for = "ENSEMBL", fetch = "GENENAME" ) %>%
...
go_overrep.up <- enrichGO(keyType = "ENSEMBL",
...
go_overrep.down <- enrichGO(keyType = "ENSEMBL",
                gene          = rownames(subset(DE_results.sorted, padj < 0.05 & log2FoldChange <= -1)),
                #universe      = rownames(DE_results.sorted),
                OrgDb         = org.Sc.sgd.db,
                ont           = "BP",
...
draw_enrichment_dotplot<- function(my_enricher_obj, 
  ...
  my_results <- my_enricher_obj@result
  my_results <- subset(my_results, p.adjust <= padj_cutoff)
  my_enricher_obj@result <- my_results
  
  my_enricher_obj@result$p.adjust <- as.numeric(format(my_results$p.adjust,  digits=3))
  
  sp <- dotplot(my_enricher_obj, 
...
draw_GSEA_barplot <- function(my_gsea_obj, 
  ...
  my_results <- my_gsea_obj@result
  
  # Filter my_results by padj_cutoff
  my_results <- subset(my_results, p.adjust <= padj_cutoff)
  
  my_results$p.adjust <- as.numeric(format(my_results$p.adjust,  digits=3))
  
  sp <- ggbarplot(head(my_results, n = my_pathway_counts), 
                  x = "Description", 
                  y = "NES",
                  fill = "p.adjust",          # change fill color by cyl
                  color = "white",            # Set bar border colors to white
...
draw_GSEA_network<- function(my_enricher_obj, # test 
  ...
  # Filter gene sets by adjusted pvalue
  my_enricher_obj@params$pvalueCutoff <- padj_cutoff
  my_enricher_obj@result <- subset(my_enricher_obj@result,
                                   my_enricher_obj@result$p.adjust <= padj_cutoff)
...
p1 <- draw_enrichment_barplot(my_enricher_obj = go_overrep.up, my_pathway_counts = 300, file_name_prefix = "./GO_overrepresentation/GO_af_vs_none_wt_UP", my_width = 10, my_height = 10, padj_cutoff = 0.05)

# Dotplot
p2 <- draw_enrichment_dotplot(my_enricher_obj = go_overrep.up, my_pathway_counts = 10, file_name_prefix = "./GO_overrepresentation/GO_af_vs_none_wt_UP", my_width = 10, my_height = 10, padj_cutoff = 0.05, font_size = 12)

p3 <- draw_enrichment_network(dds_res = DE_results_list[["af_vs_none_wt"]], my_enricher_obj = go_overrep.up, my_pathway_counts = 2000, file_name_prefix = "./GO_overrepresentation/GO_af_vs_none_wt_UP", my_width = 10, my_height = 20, padj_cutoff = 0.05)

p4 <- draw_enrichment_tree(dds_res = DE_results_list[["af_vs_none_wt"]], my_enricher_obj = go_overrep.up, my_pathway_counts = 200, file_name_prefix = "./GO_overrepresentation/GO_af_vs_none_wt_UP", my_width = 10, my_height = 20, padj_cutoff = 0.05)

# Overrepresentation analysis downregulated genes

p5 <- draw_enrichment_barplot(my_enricher_obj = go_overrep.down, my_pathway_counts = 30, file_name_prefix = "./GO_overrepresentation/GO_af_vs_none_wt_DOWN", my_width = 10, my_height = 10, padj_cutoff = 0.05)
...
p9 <- draw_GSEA_barplot(my_gsea_obj = go.gsea, file_name_prefix = "./GO_gsea/GO_af_vs_none_wt", my_width = 10, my_height = 10, my_pathway_counts = 300, padj_cutoff = 0.05)

p10 <- draw_GSEA_dotplot(my_enricher_obj = go.gsea, my_pathway_counts = 20, file_name_prefix = "./GO_gsea/GO_af_vs_none_wt", my_width = 10, my_height = 10, padj_cutoff = 0.05 , font_size = 8) ## CHECK color assignment

p11 <- draw_GSEA_network(my_enricher_obj = go.gsea, my_pathway_counts = 40, file_name_prefix = "./GO_gsea/GO_af_vs_none_wt", my_width = 10, my_height = 20, padj_cutoff = 0.05) ## CHECK Log2FC info

p12 <- draw_GSEA_tree(my_enricher_obj = go.gsea, my_pathway_counts = 200, file_name_prefix = "./GO_gsea/GO_af_vs_none_wt", my_width = 10, my_height = 30, padj_cutoff = 0.05, font_size = 9) ## CHECK Log2FC info
